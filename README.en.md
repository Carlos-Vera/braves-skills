...
10. Context checkpoint: once the session passes the percentage you choose (40% by default) Claude offers to run `/braves-save` at the first clean boundary — never mid-edit — and if tasks are left it prepares the handoff so you don't have to copy anything: the next session you open in that project picks it up on its own and carries on exactly where you were. The block is still printed for you, for when you resume from another machine or outside the project. It never saves without your yes, and it warns again every 15 points above the threshold.
